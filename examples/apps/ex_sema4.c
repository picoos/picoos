/*
 *  pico]OS semaphore example 4
 *
 *  How to use a semaphore to signal an event, with event timeout.
 *
...
#error The feature POSCFG_FEATURE_SEMAWAIT is not enabled!
#endif
...
void eventtask(void *arg)
{
  ...
  for(;;)
  {
    /* wait for event with timeout of 500ms */
    status = posSemaWait(semaphore, MS(500));

    if (status < 0)
    {
      /* should never happen */
      nosPrint("eventtask: semaphore error\n");
    }
    else
    if (status > 0)
    {
      /* timeout has been reached */
      nosPrint("eventtask: timeout\n");
    }
    else
    {
      /* status = 0, the event was triggered */
